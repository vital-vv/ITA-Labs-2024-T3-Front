...
function SmallPicture() {
  const {picturesFiles, fullValidationForm} = useSelector(state => state.lots);
  ...
  const allPictures = picturesFiles.map(picture => {
    return <img src={picture.url} alt="Small product" className={classes.smallPicture} key={uuidv4()} onClick={handleChangeBigPicture} id={picture.url}/>
  })
